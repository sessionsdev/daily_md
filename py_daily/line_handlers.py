...
class LineHandler:
    ...
    def create_header_today(self):
        ...
        lines = self.file_handler.lines

        if not self.file_handler.date_indexes.get(now.strftime("%Y-%m-%d")):
            if not lines or lines[-1] != "\n":
                lines.append("\n")
            lines.append(header)

        if lines:
            self.file_handler.write_to_file(lines)
    ...
    def append_text_today(self, text: str) -> list[str] or None:
        ...
        date_indexes = self.file_handler.date_indexes
    ...
    def get_lines_by_dates(self, dates: list[str]) -> dict[str, list[str]]:
        result = {}
        for date in dates:
            section_indexes: tuple[int, int] = self.file_handler.date_indexes.get(
                date, (None, None)
            )
            ...
            if start is None or end is None:
                continue
            result[date] = self.file_handler.lines[start:end]
        return result

    def get_lines_by_date_pattern(self, pattern: str) -> dict[str, list[str]]:
        result = {}
        for date, (start, end) in self.file_handler.date_indexes.items():
    ...
    def get_todo_lines(self):
        return [self.file_handler.lines[i] for i in self.file_handler.todo_indexes]

    def migrate_tasks_to_date(self, date: str):
        start, end = self.file_handler.date_indexes.get(date, (None, None))
        if start is None or end is None:
            raise DateHeaderNotFoundError(date)

        todo_indexes = self.file_handler.todo_indexes
        if not todo_indexes:
